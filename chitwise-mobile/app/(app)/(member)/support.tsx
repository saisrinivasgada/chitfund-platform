...
type TicketType =
  | 'CHIT' | 'DRAW' | 'PAYMENT' | 'PAYOUT' | 'ACCOUNT' | 'GENERAL';
...
function NewTicketModal({ visible, onClose, onCreate }: {
  ...
  const mut = useMutation({
    mutationFn: () => createSupportTicket({ type, subject: subject.trim(), description: description.trim(), accountCaseSubtype: type === 'ACCOUNT' ? 'PASSWORD_RECOVERY' : undefined }),
    onSuccess: (ticket) => {
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={{ flex: 1, backgroundColor: C.surface }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 16, borderBottomWidth: 1, borderBottomColor: C.gray200 }}>
          <Text style={{ fontSize: 17, fontWeight: '800', color: C.navy }}>New Support Request</Text>
...
function TicketChat({ ticket, onBack }: { ticket: any; onBack: () => void }) {
  ...
  const { data: messages = [], isLoading } = useQuery({
    queryKey: ['member-ticket-messages', ticket.id],
    queryFn: async () => {
  ...
  const sendMut = useMutation({
    ...
    onSuccess: () => {
      setText('');
      qc.invalidateQueries({ queryKey: ['member-ticket-messages', ticket.id] });
      qc.invalidateQueries({ queryKey: ['member-tickets'] });
    },
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.gray50 }}>
      ...
      <View style={{ backgroundColor: C.surface, borderBottomWidth: 1, borderBottomColor: C.gray200, padding: 14 }}>
        <TouchableOpacity onPress={onBack} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          <Text style={{ fontSize: 13, color: C.gray500 }}>All Requests</Text>
        </TouchableOpacity>
          ...
          ListEmptyComponent={
            <View style={{ alignItems: 'center', paddingVertical: 32 }}>
              ...
              <Text style={{ fontSize: 12, color: C.gray400, marginTop: 4, textAlign: 'center' }}>
                Your admin will respond here shortly.
              </Text>
            </View>
          }
          renderItem={({ item: msg }) => {
            const fromAdmin = msg.senderRole === 'ADMIN' || msg.senderRole === 'MANAGER' || msg.senderRole === 'SUPER_ADMIN';
            return (
              <View style={{ alignItems: fromAdmin ? 'flex-start' : 'flex-end' }}>
                {fromAdmin && (
                  <Text style={{ fontSize: 10, color: C.gray400, marginBottom: 2, marginLeft: 4 }}>
                    {msg.senderRole === 'SUPER_ADMIN' ? 'ChitWise Support' : 'Your Admin'}
                  </Text>
                )}
                <View style={{
                  maxWidth: '80%',
                  backgroundColor: fromAdmin ? C.white : C.navy,
                  borderRadius: 14,
                  borderBottomLeftRadius: fromAdmin ? 4 : 14,
                  borderBottomRightRadius: fromAdmin ? 14 : 4,
                  padding: 12,
                  shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3, elevation: 1,
                }}>
                  <Text style={{ fontSize: 14, color: fromAdmin ? C.gray900 : '#fff', lineHeight: 20 }}>{msg.content}</Text>
                  <Text style={{ fontSize: 10, color: fromAdmin ? C.gray400 : 'rgba(255,255,255,0.6)', marginTop: 4, textAlign: 'right' }}>
      ...
      {isClosed && (
        <View style={{ padding: 12, backgroundColor: C.surface, borderTopWidth: 1, borderTopColor: C.gray200, alignItems: 'center' }}>
          <Text style={{ fontSize: 12, color: C.gray400 }}>This request is {ticket.status.toLowerCase()}. No further replies.</Text>
        </View>
      )}
    </SafeAreaView>
  );
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.gray50 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 16, backgroundColor: C.surface, borderBottomWidth: 1, borderBottomColor: C.gray100 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          ...
          <View>
            ...
            <Text style={{ fontSize: 11, color: C.gray400 }}>Contact Your Organisation</Text>
          </View>
        </View>
        <TouchableOpacity
        ...
        >
          <Text style={{ fontSize: 13, fontWeight: '700', color: '#fff' }}>+ New Request</Text>
        </TouchableOpacity>
      </View>
      ...
      {isLoading ? (
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 32 }}>
          ...
          <Text style={{ fontSize: 18, fontWeight: '700', color: C.navy, marginBottom: 6 }}>No requests yet</Text>
          <Text style={{ fontSize: 13, color: C.gray500, textAlign: 'center', marginBottom: 24 }}>
            Have a question or issue? Send a request to your admin and they'll get back to you.
          </Text>
          <TouchableOpacity
            onPress={() => setShowNew(true)}
            style={{ backgroundColor: C.navy, borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12 }}
          >
            <Text style={{ fontSize: 14, fontWeight: '700', color: '#fff' }}>Raise a Request</Text>
          </TouchableOpacity>
        </View>
      ) : (
