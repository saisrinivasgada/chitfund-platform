...
import {
  HandCoins, History, IndianRupee, Calendar, AlertCircle,
...
const STATUS_CONFIG = {
  ...
  PARTIALLY_COLLECTED: { label: 'Partial',         variant: 'warning' },
};
...
// ─── Timeline Modal ───────────────────────────────────────────────────────────
function PickupTimelineModal({ request, memberMap, chitMap, onClose }) {
  const timeline = [
    {
      key: 'requested', icon: Clock, color: '#1E3A5F', bg: '#EEF2F8',
      label: 'Pickup Initiated', sub: 'Request created', time: request.requestedAt, done: true,
    },
    {
      key: 'assigned', icon: UserCheck, color: '#D97706', bg: '#FEF3C7',
      label: 'Assigned to You', sub: request.assignedAt ? 'You were assigned' : 'Waiting for assignment',
      time: request.assignedAt, done: !!request.assignedAt,
    },
    {
      key: 'picked_up', icon: PackageCheck, color: '#16A34A', bg: '#F0FDF4',
      label: 'Picked Up from Member', sub: request.pickedUpAt ? 'You confirmed physical pickup' : 'Not yet picked up',
      time: request.pickedUpAt, done: !!request.pickedUpAt,
    },
    {
      key: 'collected', icon: Banknote, color: '#1E3A5F', bg: '#EEF2F8',
      ...
      <div className="space-y-5 pb-2">
        <div className="flex items-center gap-3.5 px-4 py-3.5 rounded-2xl border border-gray-100 bg-gray-50">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{memberMap[request.memberId] ?? '—'}</p>
            <p className="text-xs text-gray-400 mt-0.5">{chitMap[request.chitId] ?? '—'} · ₹{fmt(request.requestedAmount)}</p>
          </div>
        </div>
              ...
              <div key={step.key} className="flex gap-3.5">
                <div className="flex flex-col items-center flex-shrink-0">
                  <div
                    ...
                    style={{ backgroundColor: step.done ? step.bg : '#F3F4F6', border: `2px solid ${step.done ? step.color : '#D1D5DB'}` }}
                  >
                    <Icon size={15} style={{ color: step.done ? step.color : '#9CA3AF' }} />
                  </div>
                  {!isLast && (
                    <div className="w-0.5 flex-1 my-1.5" style={{ backgroundColor: step.done ? step.color : '#E5E7EB', minHeight: '24px' }} />
                  )}
                </div>
                <div className={`${isLast ? 'pb-1' : 'pb-5'} min-w-0 flex-1`}>
                  ...
                  {step.time && <p className="text-xs text-gray-400 mt-1.5 font-medium">{fmtDateTime(step.time)}</p>}
                </div>
              </div>
            );
          })}
...
function TabBar({ active, onChange, count }) {
  const tabs = [
    { key: 'pickups',  label: 'Active Pickups',     icon: HandCoins, count },
    { key: 'history',  label: 'Collection History', icon: History,   count: null },
  ];
  return (
    <div className="flex border-b border-gray-200 gap-1">
      {tabs.map(({ key, label, icon: Icon, count: c }) => (
        <button
          key={key}
          onClick={() => onChange(key)}
          className={`px-5 py-2.5 text-sm font-medium border-b-2 transition-colors cursor-pointer -mb-px flex items-center gap-2 ${
            active === key ? 'border-[#1E3A5F] text-[#1E3A5F]' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        >
          <Icon size={14} />
          {label}
...
// ─── Active Pickups ───────────────────────────────────────────────────────────
function ActivePickupsTab({ memberMap, chitMap }) {
  ...
  const rescheduleMutation = useMutation({
    ...
    onSuccess: (_, vars) => {
      qc.invalidateQueries({ queryKey: ['manager-tasks'] });
      toast.success(`Rescheduled to ${vars.label ?? 'a future date'} — admin has been notified`);
      setDeferTask(null);
    },
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['manager-tasks'] });
      qc.invalidateQueries({ queryKey: ['manager-history'] });
    ...
    <div className="space-y-5">
      ...
      {tasks.length === 0 ? (
        ...
        <div className="space-y-3">
          {assignedTasks.map((task) => (
            ...
            >
              <div className="flex items-start gap-4 flex-1 min-w-0">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0" style={{ backgroundColor: '#1E3A5F' }}>
                  {(memberMap[task.memberId] ?? '?')[0].toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900 truncate">{memberMap[task.memberId] ?? task.memberId?.slice(0, 8) + '…'}</p>
                  <p className="text-sm text-gray-500 mt-0.5 truncate">{chitMap[task.chitId] ?? task.chitId?.slice(0, 8) + '…'}</p>
                  {task.notes && <p className="text-xs text-gray-400 mt-1 italic">"{task.notes}"</p>}
                </div>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="font-bold text-gray-900 flex items-center gap-0.5 justify-end text-lg"><IndianRupee size={15} />{fmt(task.requestedAmount)}</p>
                <p className="text-xs text-gray-400 mt-0.5 flex items-center gap-1 justify-end"><Calendar size={11} />Assigned {fmtDate(task.assignedAt)}</p>
              </div>
              <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                <Button variant="primary" size="sm" onClick={(e) => { e.stopPropagation(); setConfirmPickup(task); }}>
                  <PackageCheck size={14} className="mr-1" /> Mark Picked Up
                </Button>
          ...
          {pickedUpTasks.map((task) => (
            ...
            >
              <div className="flex items-start gap-4 flex-1 min-w-0">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0" style={{ backgroundColor: '#16A34A' }}>
                  {(memberMap[task.memberId] ?? '?')[0].toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900 truncate">{memberMap[task.memberId] ?? task.memberId?.slice(0, 8) + '…'}</p>
                  <p className="text-sm text-gray-500 mt-0.5 truncate">{chitMap[task.chitId] ?? task.chitId?.slice(0, 8) + '…'}</p>
                  <p className="text-xs text-green-700 mt-1 font-medium">Picked up {fmtDateTime(task.pickedUpAt)} — awaiting admin to confirm receipt</p>
                </div>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="font-bold text-gray-900 flex items-center gap-0.5 justify-end text-lg"><IndianRupee size={15} />{fmt(task.requestedAmount)}</p>
              </div>
              <span className="bg-green-100 text-green-700 text-xs font-semibold px-3 py-1.5 rounded-full flex-shrink-0">Waiting for Admin</span>
            </div>
          ))}
          ...
          {partiallyCollectedTasks.map((task) => (
            ...
            >
              <div className="flex items-start gap-4 flex-1 min-w-0">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0" style={{ backgroundColor: '#7C3AED' }}>
                  {(memberMap[task.memberId] ?? '?')[0].toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900 truncate">{memberMap[task.memberId] ?? task.memberId?.slice(0, 8) + '…'}</p>
                  <p className="text-sm text-gray-500 mt-0.5 truncate">{chitMap[task.chitId] ?? task.chitId?.slice(0, 8) + '…'}</p>
                  <p className="text-xs text-purple-700 mt-1 font-medium">Collected ₹{fmt(task.collectedAmount)} of ₹{fmt(task.requestedAmount)} — admin will follow up on remaining</p>
                </div>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="font-bold text-gray-900 flex items-center gap-0.5 justify-end text-lg"><IndianRupee size={15} />{fmt(task.collectedAmount ?? task.requestedAmount)}</p>
                <p className="text-xs text-gray-400 mt-0.5">of ₹{fmt(task.requestedAmount)} total</p>
              </div>
              <span className="bg-purple-100 text-purple-700 text-xs font-semibold px-3 py-1.5 rounded-full flex-shrink-0">Partial — Admin Follow-up</span>
            </div>
          ))}
        </div>
      )}

      {/* Partial collect dialog */}
      {editTask && (
        <Modal
          ...
          {showPartialAlert ? (
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-3 pt-1">
                <Button variant="muted" size="md" className="flex-1" onClick={() => { setEditTask(null); setEditAmount(''); }}>Cancel</Button>
                <Button
                  variant="warning" size="md" className="flex-1"
                  disabled={!editAmount || Number(editAmount) <= 0 || partialMutation.isPending}
                  loading={partialMutation.isPending}
                  onClick={() => partialMutation.mutate({ requestId: editTask.id, collectedAmount: Number(editAmount) })}
                >
                  Submit Collection
                </Button>
              </div>
      ...
      {deferTask && (
        <Modal title="Can't collect today?" onClose={() => setDeferTask(null)} size="sm">
          <div className="space-y-5">
            <div className="flex items-center gap-3.5 px-4 py-3.5 rounded-2xl border border-gray-100 bg-gray-50">
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0" style={{ backgroundColor: '#1E3A5F' }}>
            ...
            <div className="space-y-3">
              <button
                ...
                onClick={() => {
                  ...
                  d.setHours(9, 0, 0, 0);
                  rescheduleMutation.mutate({ requestId: deferTask.id, scheduledFor: d.toISOString().slice(0, 19), label: 'tomorrow' });
                }}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  d.setHours(9, 0, 0, 0);
                  rescheduleMutation.mutate({ requestId: deferTask.id, scheduledFor: d.toISOString().slice(0, 19), label: 'next week' });
                }}
              ...
              <button
                ...
                onClick={() => cancelMutation.mutate(deferTask.id)}
      ...
      {confirmPickup && (
        <ConfirmDialog
          ...
          description={`Confirm you physically collected ₹${fmt(confirmPickup.requestedAmount)} from ${memberMap[confirmPickup.memberId] ?? 'member'}? Hand the cash to admin next.`}
...
// ─── Collection History ───────────────────────────────────────────────────────
function CollectionHistoryTab({ memberMap, chitMap }) {
  const [viewTimeline, setViewTimeline] = useState(null);

  const { data: history = [], isLoading } = useQuery({
    queryKey: ['manager-history'],
          ...
          {history.map((r) => {
            ...
            return (
              <Tr key={r.id} onClick={() => setViewTimeline(r)} className="cursor-pointer hover:bg-gray-50 transition-colors">
                <Td><span className="text-sm font-medium text-gray-900">{memberMap[r.memberId] ?? r.memberId?.slice(0, 8) + '…'}</span></Td>
                <Td><span className="text-sm text-gray-600">{chitMap[r.chitId] ?? r.chitId?.slice(0, 8) + '…'}</span></Td>
                <Td><span className="flex items-center gap-0.5 font-semibold text-gray-900"><IndianRupee size={13} />{fmt(r.requestedAmount)}</span></Td>
                <Td><span className="text-sm text-gray-500">{fmtDateTime(r.updatedAt)}</span></Td>
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function ManagerCashPickupPage() {
  const [searchParams, setSearchParams] = useSearchParams();
  const tab = searchParams.get('tab') ?? 'pickups';
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      ...
      <div className="space-y-5">
        <TabBar active={tab} onChange={setTab} count={tasks.length} />

        {tab === 'pickups'  && <ActivePickupsTab memberMap={memberMap} chitMap={chitMap} />}
        {tab === 'history'  && <CollectionHistoryTab memberMap={memberMap} chitMap={chitMap} />}
      </div>
    </div>
  );
}
