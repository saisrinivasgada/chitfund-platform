...
function generatePrintHTML({ items, member, totalOwed, totalRefunded, grandTotal, tenantName = '', adjustmentAmount = 0, adjustmentReason = '' }) {
  ...
  const chitSections = items.map((item) => {
    ...
    return `<div class="chit"><div class="ch"><span class="cn">${item.chitName}</span><span class="cm">${chitStatus} · ${caseLabel[item.settlementCase] ?? item.settlementCase}${modeNote}</span></div>${payoutLine}${table}<div class="nl ${netCls}">${netLbl}</div></div>`;
  }).join('');
