...
/**
 * Refuses to start in the {@code prod} Spring profile when any critical secret
 * matches a known-compromised or known-default value.
 *
 * WHY fail at startup: a misconfigured production instance is worse than a
 * deployment failure — it silently accepts forged JWTs and spoofed internal calls.
 * Loud startup failure makes the misconfiguration impossible to miss.
 */
@Configuration
@Slf4j
public class ProductionSecretsValidator {
    ...
    public void validate() {
        ...
        if (isCompromised(jwtSecret, 32)) {
            violations.add("MANAGEMENT_JWT_SECRET is absent, too short, or a known default value");
        }
